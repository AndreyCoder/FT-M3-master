...
function problemA() {
  ...
  // promise version
  let one = promisifiedReadFile('poem-two/stanza-01.txt').then(st => blue(st))
  let two = promisifiedReadFile('poem-two/stanza-02.txt').then(st => blue(st))

  Promise.all([one, two]).then(() => console.log('done'))
}
...
function problemB() {
  ...
  // promise version
  var promises = filenames.map(el => promisifiedReadFile(el).then(st => blue(st)))
